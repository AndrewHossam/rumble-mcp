...
export const technicalToolSchemas = {
  get_technical_calls: {
    description:
      'Get a list of active technical trading calls from TheRumble. Returns short-term trading signals with entry, target, and stop-loss prices. Use get_call_details for full details on a specific call.',
  ...
  get_technical_track_record: {
    description:
      'Get the overall track record for technical calls, including win rate and performance metrics.',
    inputSchema: z.object({
      market: z.string().default('EGY').describe('Market code (default: EGY for Egypt)'),
    }),
  },
